...
# children's hospitals to be excluded for aggregation by STP
trusts_to_exclude_for_aggregation <- c("RQ3", "RBS", "RCU")

### Finding  periods of missing data

# Only check variables of interest that are not derived from other variables
...
# 5. Adding organisational information: Sustainability and Transformation Partnerships (STPs)
...
Sitrep_monthly_average_bytrust <- Sitrep_daily %>%
  ...
  summarize(occupancy.rate = mean(occupancy.rate, na.rm=TRUE))

# Monthly average on STP level
Sitrep_monthly_average_bySTP <- Sitrep_monthly_average_bytrust %>%
  filter(!is.element(code, trusts_to_exclude_for_aggregation)) %>%
  group_by(STP, STP_code, month) %>%
  summarise(occupancy.rate = mean(occupancy.rate, na.rm=TRUE))


# 7. Visualisation: creating maps in R ------------------------------------

# Load file containing STP shapes (originally from the ONS website)
# We will use the smaller, generalised version for mapping rather than the full boundaries

STP_geojson_filename <- "Sustainability_and_Transformation_Partnerships_February_2017_Ultra_Generalised_Clipped_Boundaries_in_England.geojson"
...
STP_shape_monthly <- STP_shape_monthly %>%
  ...
  filter(!is.na(STP)) %>% 
  # Turn STPs and months into factors (ignoring November, March)
...
ggsave("Bedocc_monthly_map_1819.png", map_monthly_bedocc, device = "png",  width = 650, height = 400, units = c("mm"))
